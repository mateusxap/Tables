...
{
private:
	void BubbleSort();                        //—ортировка пузырьком
	void QuickSort(int first, int last);      //Ѕыстра€ сортировка
	void MergeSort(int left, int right);      //—ортировка сли€нием
	void Merge( int left,int mid, int right); //‘ункци€ объединени€ 2 частей массива

public: 
	TSortTable(int _size=MAX_SIZE) :TScanTable(_size) {};
	void ScanToSort(TScanTable& table, SortType sortType); //ѕреобразование типа
...
inline bool TSortTable::Delete(int key)
{
	if (!Find(key))
	{
		return false;
